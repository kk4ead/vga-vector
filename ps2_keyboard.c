...
alt_u32 set_kbd_lights(bool caps, bool num, bool scroll)
{
  alt_u8 byte = 0x00;
  if (caps)   byte |= 0x04;
  if (num)    byte |= 0x02;
  ...
  int status_send = write_data_byte_with_ack(0xED, DEFAULT_PS2_TIMEOUT_VAL);
  if ( status_send == PS2_SUCCESS ) {
    status_send = write_data_byte_with_ack(byte, DEFAULT_PS2_TIMEOUT_VAL);
  }
  return status_send;
}
